...
[CmdletBinding()]
param(
    [string] $ReleaseRoot = (Join-Path (Split-Path -Parent $PSScriptRoot) '..\..\outputs\WoWS-Toolbox-v5.0.33'),
...
$setupPath = Join-Path $output 'WoWS-Toolbox-Setup-5.0.33.exe'
...
if ($launcherVersion.FileVersion.Trim() -ne '5.0.33.0' -or
    $launcherVersion.ProductVersion.Trim() -ne '5.0.33') {
