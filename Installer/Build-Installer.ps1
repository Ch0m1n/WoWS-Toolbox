...
[CmdletBinding()]
param(
    [string] $ReleaseRoot = (Join-Path (Split-Path -Parent $PSScriptRoot) '..\..\outputs\WoWS-Toolbox-v5.0.63'),
...
$setupPath = Join-Path $output 'WoWS-Toolbox-Setup-5.0.63.exe'
...
if ($launcherVersion.FileVersion.Trim() -ne '5.0.63.0' -or
    $launcherVersion.ProductVersion.Trim() -ne '5.0.63') {
